...
public class NewsDataServiceImpl implements NewsDataService {
    ...
    public List<News> getAll(User authenticatedUser) {
        var news = jsonDB.parse(COLLECTION_NAME);
        return jsonDB.filterPublicJsonViewAsList(news.stream().filter(item -> item.getPublicationStatus().contentEquals("published")
    ...
    public List<News> getAll(User authenticatedUser, int pageId) {
        ...
        return jsonDB.filterPublicJsonViewAsList(news.stream().filter(item -> (item.getPublicationStatus().contentEquals("published")
                ...
                && item.getAssociatedPage() != null && item.getAssociatedPage().getId() == pageId).toList());
    ...
    public News createOne(News newsItem, User authenticatedUser) {
        ...
        if (newsItem.getAssociatedPage() != null) {
            Page associatedPage = myPageDataService.getOne(newsItem.getAssociatedPage().getId(), authenticatedUser);
        ...
        jsonDB.serializePublicInfoOnly(allNews, COLLECTION_NAME); // Don't serialise internal fields (only public)
    ...
    public News updateOne(News newsItem, int id, User authenticatedUser) {
        ...
        if (newsItem.getShortDescription() != null)
            newsItemToUpdate.setShortDescription(StringEscapeUtils.escapeHtml4(newsItem.getShortDescription()));
